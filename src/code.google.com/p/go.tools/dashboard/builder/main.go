...
func main() {
	...
	if goroot.Exists() || *failAll {
		...
		if err := os.RemoveAll(*buildroot); err != nil {
			beego.Critical("Error removing build root (%s): %s", *buildroot, err)
		}
		if err := os.Mkdir(*buildroot, mkdirPerm); err != nil {
			beego.Critical("Error making build root (%s): %s", *buildroot, err)
		}
		var err error
		goroot, err = RemoteRepo(goroot.Master.Root, rootPath)
		if err != nil {
			beego.Critical("Error creating repository with url (%s): %s", goroot.Master.Root, err)
		}
