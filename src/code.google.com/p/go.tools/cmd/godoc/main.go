...
func handleURLFlag() {
	...
	for i := 0; i < 10; i++ {
		...
		case 301, 302, 303, 307: // redirect
			...
			if redirect == "" {
				beego.Critical("HTTP %d without Location header", w.Code)
			}
			urlstr = redirect
		default:
			beego.Critical("HTTP error %d", w.Code)
		}
	}
	beego.Critical("too many redirects")
}
...
func main() {
	...
	if httpMode {
		...
		if err := http.ListenAndServe(*httpAddr, handler); err != nil {
			beego.Critical("ListenAndServe %s: %v", *httpAddr, err)
		}
